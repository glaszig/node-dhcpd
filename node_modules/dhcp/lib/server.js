  ...
  self.on('message', function(buffer, remote) {
    ...
    if (packet.op == 1) {
      ...
      switch (parseInt(type_id)) {
        case 1: self._handle('discover', packet); break; // discover, send offer
        case 3: self._handle('request', packet); break; // request, send ack
        case 4: self._handle('decline', packet); break; // decline, notify admin
        case 7: self._handle('release', packet); break; // release, release ip address, keep record
        case 8: self._handle('inform', packet); break; // inform, send ack to ciaddr, without yiaddr/lease time
      }
...
Server.prototype._handle = function(messageType, packet) {
