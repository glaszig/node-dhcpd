...
var fromBuffer = function(b) {
  ...
  while (i < b.length && b[i] != 255) {
    var opt = parseInt(b[i]), len = b[i+1],
        s = sprintf("OPTION: %3d (%3d) %-26s", opt, len, DHCP.options[opt]);
    console.log(s);
    i = i+2;
...
var toBuffer = function(data) {
  ...
  for (var opt in data.options) {
    ...
    var encoder = optionEncoders[opt] || optionEncoders.fallback;
