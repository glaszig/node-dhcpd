...
var DhcpServer = function() {
  ...
  self.on('message', function(buffer, rinfo) {
    ...
    if (packet.op == 1) {
      ...
      util.log('Got '+type_descr+' from '+packet.chaddr+' with packet length of '+buffer.length+' bytes');
...
DhcpServer.prototype.doDiscover = function(packet) {
  ...
  packet.siaddr = this.address().address;
  ...
  this.send(offer, 0, offer.length, 68, '255.255.255.255', function(err, bytes) {
    ...
    else self.emit('offerSent', bytes, offer);
  });
